        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
            <form method="POST" action="{{ route('classes.store') }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="start_date" value="{{ old('start_date') }}" placeholder="Tanggal Mulai" class="w-full mt-1 p-2 border rounded dark:bg-gray-700 dark:text-white">
                    @error('start_date') <div class="text-red-500 text-sm mt-1">{{ $message }}</div> @enderror
                </div>

                <div class="mb-4">
                    <label class="block text-gray-700 dark:text-gray-300">Selesai</label>
                    <input type="date" name="end_date" value="{{ old('end_date') }}" placeholder="Tanggal Berakhir" class="w-full mt-1 p-2 border rounded dark:bg-gray-700 dark:text-white">
